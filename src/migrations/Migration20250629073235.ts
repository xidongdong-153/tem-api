...
export class Migration20250629073235 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table "categories" ("id" serial primary key, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "name" varchar(100) not null, "description" text not null, "sort_order" int not null default 0);`);
    ...
    this.addSql(`create table "tags" ("id" serial primary key, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "name" varchar(50) not null, "usage_count" int not null default 0);`);
    ...
    this.addSql(`create table "users" ("id" serial primary key, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "username" varchar(50) not null, "email" varchar(100) not null, "phone" varchar(20) null, "password_hash" varchar(100) not null, "nickname" varchar(100) null, "avatar" varchar(255) null, "is_active" boolean not null default true, "is_deleted" boolean not null default false, "last_login_at" timestamptz null, "last_login_ip" varchar(255) null);`);
    this.addSql(`alter table "users" add constraint "users_username_unique" unique ("username");`);
    this.addSql(`alter table "users" add constraint "users_email_unique" unique ("email");`);
    this.addSql(`alter table "users" add constraint "users_phone_unique" unique ("phone");`);

    this.addSql(`create table "articles" ("id" serial primary key, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "title" varchar(200) not null, "content" text not null, "summary" text not null, "status" text check ("status" in ('draft', 'published', 'archived')) not null default 'draft', "published_at" timestamptz null, "author_id" int null, "category_id" int null);`);
